...
### Ajouter un établissement/organisation/structure {#ajouter-element}
...
1. Renseigner le statut de l'établissement/organisation/structure :
    ...
    1. Rechercher et sélectionner le statut le plus précis possible :
        - WikidataESR propose une [liste des statuts recommandés](#listes-statuts) ;
...
### Ajouter une relation entre deux entités {#ajouter-relation}
...
la [liste des propriétés wikidataESR](#liste-proprietes).
...
### Une composante (UFR/labo) de l'établissement est manquante {#ajout-composante}

1. Vérifier l'existence d'un élément wikidata pour la composante :
    - Si aucun élément n'existe, le créer (voir [Ajouter élément](#ajouter-element)) ;
1. Depuis la page wikidata de l'établissement :
    - `Ajouter une déclaration`/`organisation filiale`, et choisir l'élément 
    wikidata de la composante (voir [Ajouter relation](#ajouter-relation)) ;
...
### Une association (COMUE, etc.) de l'établissement est manquante {#ajout-asso}

1. Depuis la page wikidata de l'établissement :
    - `Ajouter une déclaration`/`comprend`, et choisir l'élément 
    wikidata de l'association (voir [Ajouter relation](#ajouter-relation)) ;
...
### Une tutelle de l'établissement est manquante {#ajout-tutelle}

1. Depuis la page wikidata de l'établissement :
    - `Ajouter une déclaration`/`affilié à`, et choisir l'élément 
    wikidata de la tutelle (voir [Ajouter relation](#ajouter-relation))

### Une adhésion de l'établissement à une organisation est manquante {#ajout-adhesion}

1. Depuis la page wikidata de l'établissement :
    - `Ajouter une déclaration`/`membre de`, et choisir l'élément 
    wikidata de l'organisation (voir [Ajouter relation](#ajouter-relation)) ;
    - (Optionnel) Ajouter `qualitatif`/`date de début`.

### Une relation (composante/tutelle/adhésion) a pris fin {#fin-relation}
...
### Un établissement n'existe plus {#fin-etablissement}
...
1. Si l'établissement a été fusionné avec plusieurs autres pour en créer un nouveau :
    ...
    sinon le créer (voir [Ajouter élément](#ajouter-element)) ;
...
### Un établissement a changé de statut {#changement-statut}

**Ne surtout pas changer la nature de l'élément wikidata**

1. Créer un nouvel élément pour l'établissement (voir [Ajouter élément](#ajouter-element)) ;
